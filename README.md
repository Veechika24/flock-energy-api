...
- Hierarchy data is sourced from the bulk export rather than the
  per-meter `__data.json` hydration payload, since the latter is an
...
- **One shared client instance** (`functools.lru_cache`) rather than
...
- **Defensive type coercion** (`_to_float`) rather than letting Pydantic
...
- Confirm and wire up the real export endpoint, and add a fallback path
  that reconstructs hierarchy from the `__data.json` format if the export
...
The `EXPORT_ENDPOINT_PATH` TODO is a real gap - `GET /api/v1/meters/{id}`
